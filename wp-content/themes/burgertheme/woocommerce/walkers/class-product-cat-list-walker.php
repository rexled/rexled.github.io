...
class Custom_WC_Product_Cat_List_Walker extends Walker {
	...
	function start_el( &$output, $cat, $depth = 0, $args = array(), $current_object_id = 0 ) {
        ...
        $output .=  '"><a href="' . get_term_link( (int) $cat->term_id, 'product_cat' ) . '">'.((!empty($image))?'<div class="cat-image"><img src="' . $image . '" alt=" ' . __( $cat->name, 'woocommerce' ) .'" /></div>':'').'<span class="cat-name">' . __( $cat->name, 'woocommerce' ) . '</span></a>'; // insert category image into link
