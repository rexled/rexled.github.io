        ...
        if (count($terms) > 0) {
            ...
            if ($display_type == 'dropdown') {
                ...
                foreach ($terms as $term) {
                    ...
                    if (class_exists('ctTermFieldsExtension')) {

                        $termImg = ctTermFieldsExtension::getTermOption($term->term_id, 'img_url');
                        if ($termImg != false) {
                            $termImg = '<div class="cat-image"><img src="' . $termImg . '" alt="'.$term->name.'"></div>';
                        }
                    }


                    echo ($count > 0 || $option_is_set) ? '<a href="' . esc_url(apply_filters('woocommerce_layered_nav_link', $link)) . '">' . $termImg : '<span>';
